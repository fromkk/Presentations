  ...
  @unchecked Sendable
{
  ...
  func sendEvent(_ event: Event) {
    ...
    guard !mcSession.connectedPeers.isEmpty, let data = try? JSONEncoder().encode(event) else { return }
    do {
      try mcSession.send(
        data,
  ...
  func session(
    ...
    logger.info("Received data \(String(data: data, encoding: .utf8) ?? "invalid") from '\(peerID.displayName)'")
